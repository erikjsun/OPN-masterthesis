...
class CustomOPN(nn.Module):
    def __init__(self):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv2d(4*3, 96, kernel_size=11, stride=4, padding=0),
        ...
        #Pairwise Feature Extraction and Order Prediction layers are taken out
...
def load_pretrained_copn():
    model = CustomOPN()
    # Load your pre-trained weights here, e.g.:
    # model.load_state_dict(torch.load('path_to_pretrained_model.pth'))
    
    # TEMPORARY Load AlexNet weights
    alexnet = models.alexnet(pretrained=True)
    alexnet_weights = alexnet.state_dict()
    
    # Map AlexNet weights to CustomOPN
    custom_opn_dict = model.state_dict()

    # Initialize conv layers with AlexNet weights where possible
    custom_opn_dict['conv_layers.0.weight'] = alexnet_weights['features.0.weight'].repeat(4, 1, 1, 1)  # Adjusting for the 4*3 input channels
    custom_opn_dict['conv_layers.0.bias'] = alexnet_weights['features.0.bias']
    custom_opn_dict['conv_layers.4.weight'] = alexnet_weights['features.3.weight']
    custom_opn_dict['conv_layers.4.bias'] = alexnet_weights['features.3.bias']
    custom_opn_dict['conv_layers.8.weight'] = alexnet_weights['features.6.weight']
    custom_opn_dict['conv_layers.8.bias'] = alexnet_weights['features.6.bias']
    custom_opn_dict['conv_layers.10.weight'] = alexnet_weights['features.8.weight']
    custom_opn_dict['conv_layers.10.bias'] = alexnet_weights['features.8.bias']
    custom_opn_dict['conv_layers.12.weight'] = alexnet_weights['features.10.weight']
    custom_opn_dict['conv_layers.12.bias'] = alexnet_weights['features.10.bias']
    
    model.load_state_dict(custom_opn_dict)
    return model
...
class COPN_FineTune(nn.Module):
    def __init__(self, copn_model, num_classes):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(256 * 6 * 6, 2048),  # Adjust based on the output size of your conv_layers
            nn.ReLU(),
            nn.Dropout(0.5),
