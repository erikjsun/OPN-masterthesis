...
class BlobPreprocessedDataset(Dataset):
    def __init__(self, blob_service_client, container_name, all_blob_names):
        self.blob_service_client = blob_service_client
        self.container_name = container_name
        self.all_blob_names = all_blob_names
        self.datasets = []
...
def main():
    ...
    if not all_blob_names:
        ...
        print(f"Found {len(all_blob_names)} preprocessed batches in Blob Storage.")

    # Now split them into train/val sets (80/20 as an example)
    ...
    print(f"Training files: {len(train_blob_names)}")
    print(f"Validation files: {len(val_blob_names)}")
    ...
    # Create the dataset
    dataset = BlobPreprocessedDataset(blob_service_client, CONTAINERNAME, all_blob_names)
